...
- [x] **T11.4** `notification: OTLP export w/ Kafka consumer spans`  *(parallel)*
- [x] **T11.5** `observability: log-to-trace correlation (trace_id in logs + Loki derivedFields)`  *(parallel)*
- [x] **C11 — CHECKPOINT:** `CreateShipment` → distributed trace across shipment + tracking + notification in Tempo; log line in Loki clicks through to the trace
