...
const keycloak = useNuxtApp().$keycloak as
  | import("keycloak-js").default
  | undefined;
const username = ref("");
onMounted(() => {
...
async function createShipment() {
  ...
  createResult.value = "";
  try {
    const resp = await shipmentClient.createShipment(
...
async function loadShipments() {
  listError.value = "";
  try {
    const resp = await shipmentClient.listShipments(
      new ListShipmentsRequest({
...
async function reportLocation() {
  ...
  try {
    const resp = await trackingClient.reportLocation(
      new ReportLocationRequest({
...
async function getTracking() {
  ...
  try {
    const resp = await trackingClient.getTracking(
      new GetTrackingRequest({
...
<template>
  ...
  >
    ...
    <p style="color: #666">
      gRPC-Web + ConnectRPC + Nuxt 3 + @bufbuild/protobuf
    </p>
    ...
    >
      ...
      >
        <label
          >Origin City
          <input v-model="originCity" style="width: 100%; padding: 6px"
        /></label>
        <label
          >Origin Country
          <input v-model="originCountry" style="width: 100%; padding: 6px"
        /></label>
        <label
          >Dest City
          <input v-model="destCity" style="width: 100%; padding: 6px"
        /></label>
        <label
          >Dest Country
          <input v-model="destCountry" style="width: 100%; padding: 6px"
        /></label>
        <label
          >Carrier <input v-model="carrier" style="width: 100%; padding: 6px"
        /></label>
        <label
          >Weight (kg)
          <input
    ...
    >
      ...
      <p v-if="locationResult" style="margin-top: 8px; color: green">
        {{ locationResult }}
      </p>
    ...
    <footer
      ...
      "
    >
      gRPC-Web via ConnectRPC transport → Envoy (localhost:8080) → Shipment /
      Tracking services. TypeScript models generated by
      <code>buf generate</code> with <code>@bufbuild/protobuf</code> +
      <code>@connectrpc/connect-es</code>.
    </footer>
  </div>
</template>
